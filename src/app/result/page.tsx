...
export default function ResultPage() {
  ...
  const pendingResult = useRef<{ output: PlannerOutput; personsPerDay: Record<string, number> } | null>(null)
  ...
  function handleRevealDone() {
    ...
    setState({ status: 'done', ...pendingResult.current })
  }
  ...
  return (
    <AnimatePresence mode="wait">
      {state.status === 'loading' ? (
        ...
        >
          <LoadingScreen done={fetchDone} onDone={handleRevealDone} />
        </motion.div>
      ) : (
        <motion.div
          key="done"
          ...
          className="max-w-5xl mx-auto px-4 py-10 space-y-10 print:py-4 print:space-y-6"
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:grid-cols-2 print:gap-6">
            <MenuDisplay days={state.output.days} />
            <div className="lg:sticky lg:top-6 lg:self-start print:static">
              <ShoppingList departments={state.output.shopping_list} />
            </div>
          </div>
        </motion.div>
      )}
